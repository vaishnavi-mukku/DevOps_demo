...
# Get the Airflow URI
AIRFLOW_URI="https://a864faeb25904898bdabfa7b2aa0a8b6-dot-europe-west4.composer.googleusercontent.com/auth/fab/v1/roles"
...
# Create custom role
ROLE_NAME="custom_viewer"
if curl -s -X GET "${AIRFLOW_URI}/auth/fab/v1/roles?name=${ROLE_NAME}" -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" | jq '.length' | grep -q 0; then
  echo "Role ${ROLE_NAME} already exists, skipping creation."
else
  curl -X POST "${AIRFLOW_URI}/auth/fab/v1/roles" \
...
for perm in "${PERMISSIONS[@]}"; do
  curl -X POST "${AIRFLOW_URI}/auth/fab/v1/roles/${ROLE_NAME}/permissions" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H "Content-Type: application/json" \
    -d "${perm}"
done
...
if curl -s -X GET "${AIRFLOW_URI}/auth/fab/v1/users?email=${USER_EMAIL}" -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" | jq '.length' | grep -q 0; then
  # Assign the custom role to the user
  curl -X PATCH "${AIRFLOW_URI}/auth/fab/v1/users/${USER_EMAIL}" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H "Content-Type: application/json" \
    -d '{"roles": ["'${ROLE_NAME}'"]}'
else
